...
let http_handler = (req, res) => {
    ...
    if (url.parse(req.url).pathname === '/json' && req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            let jsonResponse = {};
            // data.__comment.replace("Запрос.", "Ответ.");   ///
            jsonResponse.__comment = ' Ответ: ' + data.__comment.slice(8,32); //here
    ...
    if (url.parse(req.url).pathname === '/files') {
        res.setHeader('X-static-files-count', fs.readdirSync('D:\\University\\БГТУ\\3 Курс\\5. Семестр\\NodeJS\\All Lab\\LAB_8\\static').length);
        res.end();
    }
