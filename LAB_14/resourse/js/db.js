...
const config = {
    "driver":"msnodesqlv8",
    "connectionString": "Driver={SQL Server Native Client 11.0};Server={WIN-HGMUQ1VQC5E};Database={Node_14};Trusted_Connection={yes};"
}
...
class DB {
    ...
    {
        return this.connectionPool.then(pool => {
            console.log("----postPulpits------");
            console.log(pulpit);
            console.log(pulpitName);
            console.log(faculty);
...
//full cringe 2 tasks down here, u welcome to fix this shit :)
    facultyPulpitsJoin(code){
        return this.connectionPool.then(pool => {
            ...
            .input('code', sql.NVarChar, code)
                .query('Select Pulpit.PULPIT,PULPIT.PULPIT_NAME,FACULTY.FACULTY,FACULTY.FACULTY_NAME FROM PULPIT INNER JOIN FACULTY ON PULPIT.FACULTY = @code AND  FACULTY.FACULTY  = @code');
        });
    }
    ...
    auditoriumtypesAuditoriumsJoin(code){
        return this.connectionPool.then(pool => {
            ...
            return pool.request()
            .input('code', sql.NVarChar, code)
                .query('SELECT AUDITORIUM_TYPE.AUDITORIUM_TYPE,AUDITORIUM_TYPE.AUDITORIUM_TYPENAME, AUDITORIUM.AUDITORIUM_CAPACITY, AUDITORIUM.AUDITORIUM,AUDITORIUM.AUDITORIUM_NAME FROM AUDITORIUM_TYPE INNER JOIN AUDITORIUM ON AUDITORIUM.AUDITORIUM_TYPE = @code  AND AUDITORIUM_TYPE.AUDITORIUM_TYPE = @code');
        });
    }
